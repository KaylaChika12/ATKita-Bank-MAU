...
```json
{
  "name": "inventaris-atk-backend",
  "version": "1.0.0",
  "description": "Backend for Sistem Pengelolaan Inventaris ATK",
  "main": "server.js",
  "scripts": {
...
app.use(cors()); // Mengizinkan CORS untuk komunikasi antara frontend dan backend

// Rute API
const barangRoutes = require("./routes/barangRoutes");
const pengeluaranRoutes = require("./routes/pengeluaranRoutes");
const logHistoryRoutes = require("./routes/logHistoryRoutes"); // Mengimpor logHistoryRoutes

// Menggunakan rute
app.use("/api/barang", barangRoutes);
app.use("/api/pengeluaran", pengeluaranRoutes);
app.use("/api/logHistory", logHistoryRoutes); // Pastikan ini ada
...
## File: `controllers\barangController.js`
...
const addBarang = (req, res) => {
  ...
  const stmt = db.prepare(
    "INSERT INTO barang (nama, stok, satuan) VALUES (?, ?, ?)"
  );
  stmt.run(nama, stok, satuan, (err) => {
...
module.exports = {
  ...
  getBarang,
  searchBarang,
  deleteBarang, // Menambahkan fungsi deleteBarang
  exportBarangCSV,
  exportBarangPDF,
};
```

## File: `controllers\logHistoryController.js`
...
const addLogHistory = (req, res) => {
  ...
  stmt.run(barang_masuk, barang_keluar, waktu_transaksi, user, (err) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
...
const getLogHistory = (req, res) => {
  db.all("SELECT * FROM log_history", [], (err, rows) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
...
## File: `controllers\pengeluaranController.js`
...
const addPengeluaran = (req, res) => {
  ...
  const finalTanggal = tanggal || new Date().toISOString().split("T")[0]; // Auto isi tanggal hari ini jika tidak diberikan
  ...
  stmt.run(barang_id, jumlah, finalTanggal, divisi, (err) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
...
const db = mysql.createConnection({
  host: "localhost", // MySQL server host (could be 'localhost' or an IP address)
  user: "root", // Your MySQL username
  password: "", // Your MySQL password
  database: "atkita", // The name of the database
});
...
db.connect((err) => {
  ...
  // Create the log_history table
  const createLogHistoryTable = `
  ...
  db.query(createBarangTable, (err, results) => {
    if (err) {
      console.error("Error creating barang table:", err);
      return;
    }
    console.log("Barang table created or already exists");

    // Create pengeluaran table after barang table is created
    db.query(createPengeluaranTable, (err, results) => {
      if (err) {
        console.error("Error creating pengeluaran table:", err);
        return;
      }
      console.log("Pengeluaran table created or already exists");

      // Create log_history table after pengeluaran table is created
      db.query(createLogHistoryTable, (err, results) => {
        if (err) {
          console.error("Error creating log_history table:", err);
          return;
        }
...
// const db = new sqlite3.Database("./inventaris.db"); // File database
...
## File: `routes\logHistoryRoutes.js`
...
## File: `routes\pengeluaranRoutes.js`
...
// Rute untuk mencatat pengeluaran
router.post("/", pengeluaranController.addPengeluaran);

// Rute untuk mendapatkan pengeluaran
