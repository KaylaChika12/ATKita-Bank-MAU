...
const moment = require("moment-timezone"); // Menambahkan moment-timezone untuk menangani zona waktu
...
const addBarang = (req, res) => {
  ...
  db.query(query, [nama, stok, satuan], (err, results) => {
    ...
    const currentTime = moment().tz("Asia/Jakarta"); // Mengambil waktu dengan zona waktu Asia/Jakarta
...
const deleteBarang = (req, res) => {
  ...
  db.query(deletePengeluaranQuery, [id], (err) => {
    ...
    // Setelah pengeluaran berhasil dihapus, hapus log history yang terkait dengan barang
    const deleteLogHistoryQuery =
      "DELETE FROM log_history WHERE barang_masuk LIKE ?";
    db.query(deleteLogHistoryQuery, [`%${id}%`], (err) => {
      if (err) {
        return res
          .status(500)
          .json({ error: "Gagal menghapus log history terkait." });
      }

      // Setelah log history berhasil dihapus, hapus barang
